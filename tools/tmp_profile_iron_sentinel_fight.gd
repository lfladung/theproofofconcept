...
var _bucket_sentinel_counts: Dictionary = {}
...
func _init() -> void:
	...
	if window != null:
		...
		window.title = "Iron Sentinel Fight Profiler"

	_dungeon = DUNGEON_ORCHESTRATOR_SCENE.instantiate()
	root.add_child(_dungeon)
	print("[sentinel-profile] instantiated dungeon_orchestrator")
	...
	if not floor_ready:
		...
		return
	print("[sentinel-profile] floor generated")

	if not _move_player_to_boss_room():
		_finish_with_error("Failed to move player to boss room.")
		return
	print("[sentinel-profile] player moved to boss room")

	await _wait_seconds(TELEPORT_SETTLE_SECONDS)
	await _sample_bucket("boss_room_idle", BOSS_ROOM_IDLE_SECONDS)

	print("[sentinel-profile] starting boss encounter")
	_dungeon.call("_start_boss_encounter")
...
func _move_player_to_boss_room() -> bool:
	if _dungeon == null:
		return false
	...
	var room_name := _dungeon.call("_layout_room_name", "exit_room") as StringName
	if String(room_name) == "":
		return false
...
func _sample_bucket(bucket_name: String, seconds: float) -> void:
	...
	while Time.get_ticks_usec() < end_usec:
		...
		sentinel_counts.append(_iron_sentinel_count())
		last_usec = now_usec
...
func _build_summary() -> Dictionary:
	...
	for key in _bucket_samples_ms.keys():
		...
		var sentinel_counts := _bucket_sentinel_counts.get(bucket_name, []) as Array[int]
		buckets[bucket_name] = _bucket_stats(samples, enemy_counts, sentinel_counts)
	return {
...
func _bucket_stats(samples_ms: Array[float], enemy_counts: Array[int], sentinel_counts: Array[int]) -> Dictionary:
	if samples_ms.is_empty():
		return {
			...
			"avg_enemy_count": 0.0,
			"max_enemy_count": 0,
			"avg_sentinel_count": 0.0,
			"max_sentinel_count": 0,
		}
	var total_ms := 0.0
	var max_ms := 0.0
	var over_16 := 0
	...
	var sentinel_total := 0
	var sentinel_max := 0
	for count in sentinel_counts:
		sentinel_total += count
		sentinel_max = maxi(sentinel_max, count)
	return {
		...
		"avg_enemy_count": snapped(float(enemy_total) / float(maxi(1, enemy_counts.size())), 0.01),
		"max_enemy_count": enemy_max,
		"avg_sentinel_count": snapped(float(sentinel_total) / float(maxi(1, sentinel_counts.size())), 0.01),
		"max_sentinel_count": sentinel_max,
	}
...
func _write_summary(summary: Dictionary) -> void:
	...
	if file == null:
		push_warning("[sentinel-profile] Failed to write summary to %s" % OUTPUT_PATH)
		return
	file.store_string(JSON.stringify(summary, "\t"))
	file.close()
	print("[sentinel-profile] wrote %s" % ProjectSettings.globalize_path(OUTPUT_PATH))


func _print_summary(summary: Dictionary) -> void:
	print("[sentinel-profile] summary begin")
	var buckets := summary.get("buckets", {}) as Dictionary
	for bucket_name in ["boss_room_idle", "encounter_start", "encounter_sustain"]:
		var bucket := buckets.get(bucket_name, {}) as Dictionary
		print(
			"[sentinel-profile] %s frames=%s avg_ms=%s avg_fps=%s p95_ms=%s max_ms=%s >33ms=%s >50ms=%s avg_enemies=%s max_enemies=%s avg_sentinels=%s max_sentinels=%s"
			% [
				bucket_name,
				...
				float(bucket.get("avg_sentinel_count", 0.0)),
				int(bucket.get("max_sentinel_count", 0)),
			]
		)
	print("[sentinel-profile] summary end")


func _finish_with_error(message: String) -> void:
	push_error("[sentinel-profile] %s" % message)
	quit(1)
